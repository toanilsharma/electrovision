  ...
  return (
    <div className="fixed inset-0 z-[150] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto print:p-0 print:bg-white print:static">
      <div className="w-full max-w-xl bg-slate-900 border border-slate-750 rounded-2xl shadow-2xl overflow-hidden flex flex-col my-auto print:shadow-none print:border-none print:w-full print:max-w-none print:bg-white print:text-black">
        ...
        <div className="p-4 border-b border-white/10 bg-slate-950 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-black uppercase text-white tracking-wider">
              NFPA 70E 130.5(H) Arc Flash Warning Label
            </h3>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-500 hover:bg-orange-400 text-slate-950 font-bold text-xs uppercase tracking-wide cursor-pointer transition-all shadow-md min-h-[36px]"
            >
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-750 transition-all cursor-pointer min-h-[36px]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* PRINTABLE LABEL CONTAINER (ANSI Z535 / NFPA 70E 130.5(H) COMPLIANT) */}
          ...
          <div className="border-4 border-black bg-white text-black font-sans rounded-lg overflow-hidden shadow-xl print:shadow-none print:rounded-none">
            ...
            <div className="p-4 space-y-3 font-mono">
              ...
              <div className="grid grid-cols-2 gap-3 border-2 border-black p-3 bg-yellow-50 rounded">
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider block text-slate-800">
                  ...
                  <div className="text-2xl md:text-3xl font-black tracking-tight text-black font-mono">
                    {unit === 'cal' 
                      ? `${incidentEnergy.toFixed(2)} cal/cm²` 
                      : `${incidentEnergyKjM2.toFixed(1)} kJ/m²`}
                  </div>
                </div>
              ...
              <div className="border-2 border-black p-3 bg-slate-50 rounded">
                <span className="text-[10px] font-black uppercase tracking-wider block text-slate-800">
                  NFPA 70E PPE Level / Required Rating
                </span>
                <div className="text-base md:text-lg font-black uppercase text-black font-mono">
                  {hrcLevel === 5 ? (
                    <span className="text-red-700 font-extrabold">DANGER: &gt;40 cal/cm² — LIVE WORK STRICTLY PROHIBITED</span>
                  ) : (
                    <span>PPE Category {hrcLevel} (Min. {hrcLevel === 1 ? '4' : hrcLevel === 2 ? '8' : hrcLevel === 3 ? '25' : '40'} cal/cm² Arc Suit Ensemble)</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 border-2 border-black p-3 bg-white rounded text-xs font-bold">
                <div>
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider block text-slate-600 mt-2">
              ...
              {/* Equipment Info & Footer Metadata per NFPA 70E 130.5(H) */}
              <div className="flex justify-between items-center text-[10px] font-mono font-bold text-slate-700 border-t-2 border-black pt-2">
                <div>
                  <span>Equipment: <strong>{equipmentId}</strong></span>
        ...
        <div className="p-3 bg-slate-950 border-t border-white/10 text-center text-xs text-slate-400 font-mono print:hidden">
          Standardized label format compliant with NFPA 70E 2024 Section 130.5(H) &amp; IEEE 1584-2018.
        </div>
