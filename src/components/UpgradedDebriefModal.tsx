...
export const UpgradedDebriefModal: React.FC<UpgradedDebriefModalProps> = ({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center lg:justify-start p-2 sm:p-4 lg:pl-6 pointer-events-none overflow-y-auto">
          {/* Backdrop layer: full screen on mobile; on laptop scoped to left/center so human twin on right stays uncovered */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 lg:right-[500px] xl:right-[560px] bg-slate-950/80 backdrop-blur-md pointer-events-auto"
          />
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-4xl lg:max-w-[calc(100vw-530px)] xl:max-w-[calc(100vw-590px)] bg-slate-950 border-2 border-slate-800 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.9)] flex flex-col max-h-[90vh] overflow-hidden select-none pointer-events-auto my-auto"
          >
            {/* Modal Top Header */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col text-left">
                  ...
                  <span className="text-[10px] sm:text-xs font-mono font-bold text-slate-400 mt-0.5">
                    Incident Touch Potential: {voltage}V | Path: {shockPath} | Duration: {durationMs}ms
                  </span>
                </div>
              </div>
              <button
            ...
            {/* RCD Protection Special Banner (if active/tripped) */}
            {rcdTripped && (
              <div className="bg-emerald-950/90 border-b-2 border-emerald-500/80 p-3 flex items-center gap-3">
                <CheckCircle2 className="w-6 h-6 text-emerald-400 shrink-0 animate-bounce" />
                <div className="flex flex-col text-left">
                  <span className="text-xs font-black uppercase tracking-wider text-emerald-300">
                    🛡️ RESIDUAL CURRENT DEVICE (RCBO / ELCB) SAVED YOUR LIFE!
                  </span>
                  <span className="text-[11px] font-mono font-bold text-emerald-100">
                    Device sensed {currentMA.toFixed(1)}mA prospective earth leakage and tripped open within {rcdTripTimeMs || 28}ms (well within IEC 61008 limit of 40ms). Fatal heart fibrillation prevented!
                  </span>
                </div>
              </div>
            )}
