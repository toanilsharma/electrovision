...
export function DisclaimerModal({ onAccept }: DisclaimerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-between p-2 sm:p-4 pb-14 bg-[#020617]/90 backdrop-blur-xl overflow-y-auto">
      <div className="w-full flex-1 flex items-center justify-center my-auto py-2">
        <motion.div
          initial={{ opacity: 0, scale: 0.96, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          className="w-full max-w-md lg:max-w-4xl xl:max-w-5xl overflow-hidden bg-slate-900 border shadow-[0_0_50px_rgba(249,115,22,0.2)] border-orange-500/30 rounded-2xl flex flex-col max-h-[calc(100vh-5.5rem)]"
        >
          {/* Top Commercial Tool Header */}
          <div className="px-4 py-3 sm:px-6 sm:py-4 border-b bg-gradient-to-r from-slate-900 via-slate-900/90 to-orange-950/30 border-white/10 flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="p-3 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto flex-1 min-h-0 custom-scrollbar">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="p-3.5 sm:p-5 rounded-xl bg-slate-950/60 border border-white/10 hover:border-slate-700 transition-colors flex flex-col justify-between">
                <div>
          ...
          <div className="p-3.5 sm:p-5 bg-slate-950/95 border-t border-orange-500/30 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 shrink-0 shadow-inner">
            <button
              type="button"
              onClick={() => setAccepted(!accepted)}
              className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all cursor-pointer select-none w-full sm:w-auto flex-1 text-left ${
                accepted 
            ...
            >
              ...
              <span className={`text-xs sm:text-sm font-bold transition-colors leading-tight ${
                accepted ? 'text-orange-300' : 'text-slate-200'
              }`}>
            ...
            <button
              ...
              disabled={!accepted}
              onClick={onAccept}
              className={`w-full sm:w-auto px-7 sm:px-9 py-3.5 sm:py-4 font-black text-xs sm:text-sm uppercase tracking-widest rounded-xl transition-all duration-300 flex items-center justify-center gap-2.5 shrink-0 ${
                accepted
                  ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-slate-950 hover:from-orange-400 hover:to-amber-400 shadow-[0_0_35px_rgba(249,115,22,0.6)] hover:scale-[1.03] active:scale-[0.97] cursor-pointer ring-2 ring-orange-400/50'
                  : 'bg-slate-800/80 text-slate-400 border border-white/10 cursor-not-allowed opacity-60'
              }`}
            >
      ...
      <div className="fixed bottom-0 left-0 right-0 p-2 bg-slate-950/95 border-t border-slate-800 text-center text-[9px] sm:text-[10px] text-white font-black uppercase tracking-widest z-50 flex flex-wrap items-center justify-center gap-x-2 gap-y-0.5 pointer-events-auto">
