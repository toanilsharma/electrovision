...
import { TCCChart } from '../charts/TCCChart';
...
export interface HazardConsoleProps {
...
export const HazardConsole: React.FC<HazardConsoleProps> = ({
  snapshot,
  In,
  curve,
  faultCurrent,
    ...
    if (isCurrentlyOverloaded) {
      if (!overloadSessionRef.current.active) {
        ...
        setFeedItems((prev) =>
          prev.map((item) => {
            if (item.id === activeId) {
              const baseDetail = item.detail.split(' | Duration:')[0];
              return {
                ...item,
